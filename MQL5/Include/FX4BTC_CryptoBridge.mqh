...
bool Binance_GetOpenOrders(string sym);
#import
...
bool Binance_US_GetOpenOrders(string sym);
#import
...
bool BinanceFutures_GetOpenOrders(string sym);
...
bool Bybit_GetOpenOrders(string sym);
...
bool Bitmex_GetOpenOrders(string sym);
bool Bitmex_Position(string sym);
bool Bitmex_Min_Notional_Lotsize(double bybit_notional_min,double ask_bid,int lotDigit);
...
input int    Exchange_RateLimiterDelay          = 5000;
...
public:
   ...
   bool              GetBalance(string sym,string quote_base,int ExchangeNumber);
   ...
   bool              Get_OpenOrders(string sym,int ExchangeNumber);
   ...
   margin trading
   */
   bool              Margin_Close_Position(string sym,string orderType,int ExchangeNumber);
...
bool CryptoBridge::Open_Trade_Stop(string sym,string side,string orderType,double orderSize,double orderPrice,int ExchangeNumber)
  {
   ...
   if(ExchangeNumber==1)
    ...
    if(ExchangeNumber==5)
     {
      BinanceFutures_Open_Trade_Stop(sym,side,orderType,orderSize,orderPrice);
     }
     if(ExchangeNumber==6)
     {
      Binance_US_Open_Trade_Stop(sym,side,orderType,orderSize,orderPrice);
     }
...
bool CryptoBridge::Open_Trade_StopLimit(string sym,string side,string orderType,double orderSize,double orderPrice,double stopPrice,int ExchangeNumber)
  {
   ...
   if(ExchangeNumber==1)
    ...
    if(ExchangeNumber==5)
     {
      BinanceFutures_Open_Trade_StopLimit(sym,side,orderType,orderSize,orderPrice,stopPrice);
     }
      if(ExchangeNumber==6)
     {
      Binance_US_Open_Trade_StopLimit(sym,side,orderType,orderSize,orderPrice,stopPrice);
     }
     return(false);
  }
//+------------------------------------------------------------------+
...
//|                                                              
//+------------------------------------------------------------------+ 
bool CryptoBridge::Get_OpenOrders(string sym,int ExchangeNumber)
{
    ...
    CryptoBridge::Delete_SubWindow_Object_All(0, "orderid");//    - this is the order edit button "X" to cancel individual orders
    /*
    Delete_Global_Prefix( exchangename + "_Order")
    */
   ...
   if(ExchangeNumber==2)
     {
      return( Bybit_GetOpenOrders(sym) );
     }
   if(ExchangeNumber==3)
     {
     ...
     if(ExchangeNumber==6)
     {
      return( Binance_US_GetOpenOrders(sym) );
     }
      if(ExchangeNumber==7)
     {
...
bool CryptoBridge::Margin_Close_Position(string sym,string orderType,int ExchangeNumber)
  {
...
//+------------------------------------------------------------------+ 
bool CryptoBridge::GetBalance(string sym,string quote_base,int ExchangeNumber)
  {
   ...
   if(ExchangeNumber==4)
     ...
     if(ExchangeNumber==5)
     {
      ...
      if(ExchangeNumber==6)
     {
      Binance_US_Balance(sym,quote_base);
      return(true);
     }
      if(ExchangeNumber==7)
     {
      Deribit_Balance(sym,quote_base);
      return(true);
     }
...
void CryptoBridge::Parse_Orders(string exchangeName, int order_location, int id_location)
  {
   ...
   if(counterC>0)
     {
      ArrayResize(myarray,counterC,0);
      for(int i=0; i<counterC; i++)
        {
         Set_Sub_Window_Text("sub_order_"+IntegerToString(i),myarray[i],order_location,20+(25*i),Gray,12);
        }
         ...
         if("LIQ"==wall && dash3!=-1)
          ...
          string  liq_info=StringSubstr(name,dash2+1,-1);// DONT INCLUDE THE DASH2
           ...
           // Print("LIQ Dash4 (symbol) : " + symbol);
            ...
            string liq_value =DoubleToString(liq,qt_precision);
            liquidationarray[counterE]=liq_value; 
      ...
      for(int i=0; i<counterE; i++)
        {
          ...
          Set_Sub_Window_Text("sub_liq_"+IntegerToString(i),liquidationarray[i],liq_location,20+(25*i),Gray,10);
   ...
   if(find>=0)// Object found in window 0 or window 1
     {
         ...
         if(ObjectSetString(0,name,OBJPROP_TEXT,text))
           {
            // Print("Set_Sub_Window_Text() Text Updated: "+text);
               ...
               if(dev_debug)   Print(" objectSetString failed; Error(): "+IntegerToString(GetLastError())  );
           }
        }
    }else{
      //+------------------------------------------------------------------+
      ...
      if(ObjectCreate(0,name,OBJ_LABEL,1,0,0))
       ...
       if(dev_debug)   Print(" object created: "+name);
           }else{
       if(dev_debug)   Print(" object creation failed; Error(): "+IntegerToString(GetLastError()));
        }
     }
...
void CryptoBridge::Delete_Global_Prefix(int exchangeNumber){
  ...
  if(dev_debug) Print(" Delete_Global_Prefix(int) :"+ value+ " prefix: " + prefix);
}
/*
...
void CryptoBridge::Delete_Global_Prefix(string prefix){
  bool dev_debug = true;
  int value = GlobalVariablesDeleteAll(prefix,0);
  if(dev_debug) Print(" Delete_Global_Prefix(string) :"+ value + " prefix: " + prefix);
}
/*
   ...
   for(int i=0; i<loop; i++)
     {
      ...
      if(ObjectDelete(id_order,objectname+IntegerToString(i))==false)
        {
       if(dev_debug)  Print("_MT5_Object_Delete Failed Error: "+IntegerToString(GetLastError() ));
        }
     }
  }
   ...
   for(int i=0; i<loop; i++)
     {
        ...
        {
           if(dev_debug)  Print("Delete_SubWindow_Object_Name: Failed "+IntegerToString(GetLastError() ));
        }else{
        
           if(dev_debug)  Print("Delete_SubWindow_Object_Name: Success "+" objectname: "+objectname);
           break;
...
/*
deletes one globale variable based on the exchange name "Bybit" and the double value of the global ( the order # 1,2,3,4,5,6,7,8,)
     ...
     string wall = StringSubstr(wal,0,5);// Order"
      if("Order" == wall && dash3 != -1){
        ...
        string orderstring = StringSubstr(wal,dash3+1,-1);
          if(dev_debug)   Print("Working with orderstring; Symbol - OrderType - Side - Price - Volume " +orderstring);
          
          int dash4 = StringFind(orderstring,"_",0); // SCAN AFTER THE PREVIOUS DASH
           string symbol = StringSubstr(orderstring,0,dash4);// Symbol
            if(dev_debug)   Print("Find symbol: "+ sym + " Working with string : " + symbol);
               if( GlobalVariableGet(name) == value && symbol == sym){
               
                 GlobalVariableDel(name);
                 if(dev_debug)   Print("Delete Global Name: " + name);
